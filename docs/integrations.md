...
- For Feishu/Lark stdin-based hooks, use:
  ...
  /path/to/coding-agent-notifier/scripts/notifier/lark_notify.py
  ```
- Codex has a `Stop` hook, but it needs the hook-trust step and does not fire on an interrupted or failed turn. `notify = ["/path/to/scripts/notifier/agent_notify_wrapper.sh"]` in `~/.codex/config.toml` needs no trust and is the simpler wiring; see "Message format" in `README.md`. Codex's other hook events still belong in `~/.codex/hooks.json`. Keep credentials in `~/.codex/config.toml` under `[shell_environment_policy.set]`, or in a 0600 user-level env file that the hook command loads explicitly.
